...
Phase 4 rollback order is `007_patch_verification.sql`, `006_patch_mutations.sql`, then `005_patch_state.sql`. This removes deterministic verification and fixed mutation procedures before dropping revision/audit-head state. Existing immutable asset versions, decisions, verification rows, mutation replays, and audit events are Phase 2 foundation data and are deliberately retained unless migration `001` is also rolled back.
